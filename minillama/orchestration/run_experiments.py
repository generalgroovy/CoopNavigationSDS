...
from minillama.assistant.config import RUN_MODE, SPEECH_ASR_ENGINE, SPEECH_AUDIO_DIR, SPEECH_ENGINE, SPEECH_INCOMING_ENABLED, SPEECH_OUTGOING_ENABLED, SPEECH_PLAYBACK_ENABLED, SPEECH_REALTIME_ENABLED, SPEECH_SCOPE, SPEECH_TTS_ENGINE, speech_pattern_keys
from minillama.assistant.plugin_registry import AgentBPluginConfig
from minillama.caller.config import PERSONAS
from minillama.orchestration.config import AGENT_A_TRANSFER_TOLERANCE, NUM_TURNS, RESULTS_DIR, SESSION_LOG_PROFILE
from minillama.orchestration.runner import ExperimentRunner, build_condition_grid, write_metrics_file
from minillama.analysis.research_artifacts import create_execution_run_dir, run_scoped_path, write_conversation_protocols, write_experiment_manifest, write_metric_phase_logs, write_network_research_artifacts
...
def main():
    ...
    parser.add_argument("--speech-audio-dir", default=SPEECH_AUDIO_DIR or None, help="Directory for generated speech wave files and transcript artifacts.")
    ...
    print(f"wrote {len(protocol_paths)} conversation protocols to {protocol_log_dir}")
