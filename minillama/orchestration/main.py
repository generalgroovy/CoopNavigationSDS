...
from minillama.caller.config import DEFAULT_PERSONA, LLM_AGENT_A, PERSONAS
...
from minillama.assistant.plugin_registry import AgentBPluginConfig, available_agent_b_plugin_keys, create_agent_b_plugin
from minillama.speech.io import SpeechPipelineConfig, SpeechPipelineError, SpeechTransport
from minillama.orchestration.config import (
...
from minillama.interface.config import GUI_REFRESH_MS
from minillama.orchestration.dialog_manager import DEFAULT_MAX_TURN_ELAPSED_SEC, HARD_MAX_TURN_ELAPSED_SEC, DialogManager
from minillama.orchestration.session_logging import MonitoringEventQueue, SessionLogger
from minillama.analysis.metrics import DEFAULT_METRIC_CONFIG, metric_config_with_defaults
from minillama.analysis.research_artifacts import create_execution_run_dir, run_scoped_path, write_single_run_research_outputs, write_network_research_artifacts
from minillama.network.config import GENERATION_MAX_TIME_SEC, MAX_INPUT_TOKENS, MAX_NEW_TOKENS, MODEL, MODEL_PROVIDER
from minillama.network.route_constraints import OBJECTIVE_MODES, normalize_objective_mode
from minillama.scenarios.config import DEFAULT_TEST_CASE
...
def select_run_config():
    ...
    try:
        from minillama.interface.gui import StartupConfigDialog
...
def main():
    ...
    network_output_dir = Path(run_config["execution_run_dir"]) if run_config.get("execution_run_dir") else Path(PROTOCOL_LOG_DIR)
    network_picture_dir = network_output_dir
    ...
    ui_queue = queue.Queue()
    session_log_dir = Path(run_config["execution_run_dir"]) if run_config.get("execution_run_dir") else Path(SESSION_LOG_DIR)
    ...
    if not agent_b_config.needs_model and not run_config.get("llm_agent_a", LLM_AGENT_A):
        ...
        try:
            from minillama.network.model_runtime import create_model_adapter
