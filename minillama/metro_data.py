...
from minillama.station_names import get_station_names
...
def compact_line_fullness_text():
    """Compact current passenger load text for prompts."""
    return " ".join(
        f"{line}:{data.get('fullness', 0)}% full."
        for line, data in LINES.items()
    )
