...
from minillama.config import DEVICE, MODEL, MODEL_PROVIDER, TOKEN
from minillama.model_adapters import OpenAICompatibleChatAdapter, TransformersModelAdapter
...
def load_model_and_tokenizer(model_name: str = MODEL, token: str | None = TOKEN, device: str = DEVICE):
    ...
    """
    if token:
        # Best-effort authentication. Some environments (sandboxes, CI) block
        # outbound network connections, and login() may attempt a whoami call.
        try:
            login(token=token, add_to_git_credential=False)
        except Exception as exc:
            logging.warning("HF login skipped: %s", exc)
    else:
