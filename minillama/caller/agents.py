...
from minillama.caller.config import HISTORY_MESSAGES
from minillama.caller.prompting import (
    build_agent_a_system,
    build_agent_b_system,
    generate_agent_a_template,
)
from minillama.network.metro_data import STATION_POS
from minillama.network.model_adapters import ChatMessage, messages_to_prompt
...
def fallback_reply(active_agent_name, scenario, route_index=0, persona=None, conversation=None):
    ...
    if active_agent_name == "Agent B":
        ...
        if steps:
            from minillama.network.route_constraints import probability_class, route_delay_probability, route_has_near_capacity, route_transfer_miss_probability
