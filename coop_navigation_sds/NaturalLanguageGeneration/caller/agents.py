...
from coop_navigation_sds.NaturalLanguageGeneration.caller.config import HISTORY_MESSAGES
...
from coop_navigation_sds.NaturalLanguageGeneration.models import ChatMessage, messages_to_prompt
from coop_navigation_sds.TransportNetwork.routes import (
    optimal_time_route,
    route_line_change_count,
    route_text_from_steps, fmt_time,
)
...
def initial_conversation(scenario):
    ...
    return [
        (
            "Agent A",
            (
                f"Hi, I'm starting at station {scenario['start_station']} at {fmt_time(scenario['start_time_min'])}. "
                f"I need to take transit lines to get to {scenario['destination_station']}. "
                "Which lines should I take?"
            ),
        )
    ]
    ...
    if active_agent_name == "Agent B":
        ...
        if steps:
            ...
            if agent_a_requested_secondary_constraints(conversation or []):
                ...
                if any(term in latest_request for term in ("crowd", "capacity", "packed", "full")):
                    return f"{snippet} It is {capacity}."
                if any(term in latest_request for term in ("transfer miss", "safer transfer")):
                    return f"{snippet} Transfer miss risk is {transfer_risk}."
                if any(term in latest_request for term in ("delay", "reliable", "risk")):
                    return f"{snippet} Delay risk is {delay_risk}."
            return snippet
        return (
