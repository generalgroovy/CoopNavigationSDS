...
class SessionLogger:
    ...
    def _format_summary(event: dict[str, Any]) -> str | None:
        ...
        if kind == "system":
            if name == "telemetry.phase_timing":
                return (
                    f"[{timestamp}] TIMING turn={payload.get('turn')} speaker={payload.get('speaker')} "
                    f"generation={payload.get('natural_language_generation_sec')} "
                    f"synthesis={payload.get('text_to_speech_processing_sec')} "
                    f"audio={payload.get('audio_duration_sec')} "
                    f"recognition={payload.get('automatic_speech_recognition_processing_sec')} "
                    f"understanding={payload.get('natural_language_understanding_sec')} "
            ...
            if str(name).startswith("phase."):
                return (
                    f"[{timestamp}] PHASE turn={payload.get('turn')} "
                    f"speaker={payload.get('speaker')} phase={payload.get('phase')} "
                    f"duration_sec={payload.get('latency_sec')}"
                )
    ...
    def accepts_tuple_event(self, event) -> bool:
        ...
        if self.profile == LOG_PROFILE_STARTUP:
            return False
        kind = event[0]
        return kind in {"message", "system", "warning", "route", "candidate", "phase", "telemetry", "metric_snapshot", "metrics", "timer_start", "done"}
...
class MonitoringEventQueue:
    ...
    def _to_structured_event(event):
        ...
        if kind == "phase" and len(event) >= 2:
            payload = event[1] if isinstance(event[1], dict) else {"value": event[1]}
            phase = str(payload.get("phase", "unknown")).lower()
            turn = payload.get("turn", "?")
            speaker = str(payload.get("speaker", "agent")).lower().replace(" ", "_")
            return SystemEvent(
                "ui-forward",
                timestamp,
                f"phase.{turn}.{speaker}.{phase}",
                payload=payload,
            )
