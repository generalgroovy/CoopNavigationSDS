...
class StandardizedTestCase:
    ...
    def scenario(self):
        ...
        Returns:
            The computed value or side effect documented by the implementation.
        """
        scenario = get_scenario(self.scenario_key)
        scenario.update(self.scenario_overrides or {})
        return scenario
    ...
    def opening_utterance(self) -> str:
        ...
        if len(destinations) > 1:
            ...
            return (
                f"I'm starting at station {scenario['start_station']} at {fmt_time(scenario['start_time_min'])}. "
                f"First to {scenario['destination_station']}, then {later_destination_text}. Which route should I take first?"
            )
        return (
            f"I'm starting at station {scenario['start_station']} at {fmt_time(scenario['start_time_min'])}. "
            f"I need to take transit lines to {scenario['destination_station']}. Which lines should I take?"
        )
    ...
    def with_scenario_overrides(self, **values):
        """Return a copy whose runtime policy values override scenario defaults."""
        return replace(
            self,
            scenario_overrides={
                **(self.scenario_overrides or {}),
                **values,
            },
        )
