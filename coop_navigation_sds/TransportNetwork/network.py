...
from coop_navigation_sds.Configuration.travel import (
    ...
    MIN_TRAVEL_TIME_MIN,
    MAX_TRAVEL_TIME_MIN,
    LINE_TRAVEL_MINUTES_PER_MAP_UNIT,
...
from coop_navigation_sds.TransportNetwork.station_names import get_station_names
...
def _choose_legacy_lines(stations, seed=None):
    ...
    # Prefer a ring first because it makes a useful outer connector.
    ...
    ensure_core_service(lines, stations)
...
def station_public_mode_map(stations, seed=0):
    ...
    offset = int(seed or 0) % len(pairs)
    return {station: pairs[(index + offset) % len(pairs)] for index, station in enumerate(stations)}


def choose_lines(stations, seed=None):
    """Build deterministic multimodal services with two public modes per station."""
    station_modes = station_public_mode_map(stations, seed)
    candidates = make_candidate_lines(stations)
    lines = {}
    mode_line_counts = {mode: 0 for mode in PUBLIC_TRANSPORT_MODES}

    def next_line_name(mode):
        mode_line_counts[mode] += 1
        if mode_line_counts[mode] > MODE_LINE_LIMITS[mode]:
            raise ValueError(
                f"Generated {mode} line count exceeds {MODE_LINE_LIMITS[mode]}."
            )
        return f"{MODE_LINE_PREFIXES[mode]}{mode_line_counts[mode]}"

    def add_line(_candidate_name, kind, mode, stops):
        supported = [
            station for station in stops
            if mode in station_modes[station]
        ]
        supported = list(dict.fromkeys(supported))
        if len(supported) < MIN_LINE_STATIONS:
            return False
        name = next_line_name(mode)
        index = len(lines)
        lines[name] = {
            "kind": kind,
            "mode": mode,
            "color": LINE_COLORS[index % len(LINE_COLORS)],
            "headway": MODE_HEADWAY_MIN[mode],
            "stops": supported,
        }
        return True

    # Trunk services guarantee two public modes at every station and connect
    # the three overlapping mode groups.
    add_line("Ring", "Ring", "metro", stations)
    add_line("Tram trunk", "Tram", "tram", stations)
    add_line("Bus trunk", "Bus", "bus", stations)

    mode_cycle = ("metro", "tram", "bus")
    for index, (name, kind, stops) in enumerate(candidates):
        if len(lines) >= NUM_LINES - 1:
            break
        if name == "Ring":
            continue
        preferred = mode_cycle[index % len(mode_cycle)]
        ordered_modes = (preferred,) + tuple(mode for mode in mode_cycle if mode != preferred)
        for mode in ordered_modes:
            if add_line(name, kind, mode, stops):
                break

    # Walking is an additional local mode and does not count toward the
    # exactly-two-public-modes station invariant.
    lines["Walking"] = {
        "kind": "Walking",
        "mode": WALKING_MODE,
        "color": "#6b7280",
...
def ensure_core_service(lines, stations):
    ...
    covered = {station for data in lines.values() for station in data["stops"]}
    if set(stations) - covered:
        add_line("Core Line", "Connector", stations, DEFAULT_HEADWAY_MIN + 2)
...
def line_delay_probability_class(line_name):
    ...
    if mode == WALKING_MODE:
        return "low"
    score = {"metro": 0, "tram": 1, "bus": 2}.get(mode, 1)
    if fullness == "high":
        score += 1
    if score >= 3:
        return "high"
...
def travel_time_for_line(line_name, station_a, station_b, rng=None):
...
STATION_PUBLIC_MODES = station_public_mode_map(STATIONS, NETWORK_SEED)
...
def assert_station_mode_invariant():
    """Fail fast when generated public service modes violate the experiment design."""
    observed = {
        station: {
            data["mode"]
            for data in LINES.values()
            if data.get("mode") in PUBLIC_TRANSPORT_MODES and station in data["stops"]
        }
        for station in STATIONS
    }
    invalid = {station: modes for station, modes in observed.items() if len(modes) != 2}
    if invalid:
        raise RuntimeError(f"Every station must have exactly two public transport modes: {invalid}")
    if {mode for modes in observed.values() for mode in modes} != set(PUBLIC_TRANSPORT_MODES):
        raise RuntimeError("Metro, tram, and bus must all occur in the generated network.")


assert_station_mode_invariant()

ACTIVE_NETWORK_SEED = int(NETWORK_SEED)


def rebuild_network(seed=NETWORK_SEED, force=False):
    """Rebuild mutable network data for a reproducible experiment seed."""
    global ACTIVE_NETWORK_SEED
    seed = int(seed)
    if not force and seed == ACTIVE_NETWORK_SEED:
        return seed
    new_lines = choose_lines(STATIONS, seed)
    apply_line_fullness(new_lines, seed + 42)
    new_travel_times = generate_travel_times(new_lines, seed)
    new_edges, new_adjacency = build_edges_and_adjacency(STATIONS, new_lines, new_travel_times)
    new_transfer_times = build_station_transfer_times(STATIONS, new_lines, seed + 126)
    new_profiles = build_station_profiles(STATIONS, new_lines, seed + 84)
    LINES.clear(); LINES.update(new_lines)
    TRAVEL_TIMES.clear(); TRAVEL_TIMES.update(new_travel_times)
    EDGES.clear(); EDGES.update(new_edges)
    ADJACENCY.clear(); ADJACENCY.update(new_adjacency)
    STATION_TRANSFER_TIMES.clear(); STATION_TRANSFER_TIMES.update(new_transfer_times)
    STATION_PROFILES.clear(); STATION_PROFILES.update(new_profiles)
    STATION_PUBLIC_MODES.clear(); STATION_PUBLIC_MODES.update(station_public_mode_map(STATIONS, seed))
    station_fullness_percent.cache_clear()
    line_fullness_percent.cache_clear()
    for line_name in LINES:
        LINES[line_name]["fullness"] = line_fullness_percent(line_name, START_TIME_MIN)
        LINES[line_name]["fullness_class"] = fullness_class(LINES[line_name]["fullness"])
        LINES[line_name]["delay_probability_class"] = line_delay_probability_class(line_name)
    from coop_navigation_sds.TransportNetwork import routes
    routes.optimal_time_route.cache_clear()
    routes.candidate_time_routes.cache_clear()
    compact_network_text.cache_clear()
    compact_line_service_text.cache_clear()
    compact_station_transfer_text.cache_clear()
    compact_line_fullness_text.cache_clear()
    compact_station_crowding_text.cache_clear()
    compact_travel_time_text.cache_clear()
    assert_fully_connected()
    assert_station_mode_invariant()
    ACTIVE_NETWORK_SEED = seed
    return seed
...
@lru_cache(maxsize=1)
def compact_line_service_text():
    """Return compact line-service context."""
    return " ".join(f"{line}: {data.get('kind', 'Line')}." for line, data in LINES.items())
